...
function [X,Rn] = find_soft_clusters_X(Z,R,a,show1,show2)
  ZR1 = Z*R;   %  an N x K matrix
  K = size(ZR1,2);
  Rn = eye(K);
  if show1 == 1
     fprintf('Z*R in find4_X \n')
     ZR1
  end
  [ZR2, Rn] = colpos(ZR1);   %  to make the sum of each column nonnegative
  if show2 == 1
    % fprintf('Z*R and Rn after colpos in find3_X \n')
     fprintf('Rn after colpos in find4_X \n')
    % ZR2
     Rn
  end
  for st = 1:2
      ...
      if show1 == 1
         fprintf('X1 in find_soft_clusters_X, and st = %d \n',st)
         X1
      end
      if st == 1
